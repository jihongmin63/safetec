...
Proof.
  ...
  unfold t_update. rewrite <- String.eqb_eq in H.
...
Proof.
  ...
  apply functional_extensionality. unfold t_update. intro y.
  destruct (String.eqb_spec x1 y).
  - destruct (String.eqb_spec x2 y).
    + rewrite e in H. rewrite e0 in H. assert (contra : y = y). { reflexivity. } apply H in contra. destruct contra.
    + reflexivity.
  - destruct (String.eqb_spec x2 y).
    + reflexivity.
    + reflexivity.
